...
def mnist_pipeline():
    ...
    data_0 = dsl.ContainerOp(
        ...
        image="molozise/mnist-pre-data:latest",
    ...
    data_1 = dsl.ContainerOp(
        ...
        image="molozise/mnist-val-data:latest",
    ).set_display_name('validate data').after(data_0)\
    .apply(onprem.mount_pvc("data-pvc", volume_name="data", volume_mount_path="/data"))

    train_model = dsl.ContainerOp(
        name="train embedding model",
        image="molozise/mnist-train-model:latest",
    ...
    embedding = dsl.ContainerOp(
        ...
        image="molozise/mnist-embedding:latest",
    ...
    train_faiss = dsl.ContainerOp(
        name="train faiss",
        image="molozise/mnist-train-faiss:latest",
    ...
    analysis = dsl.ContainerOp(
        name="analysis total",
        image="molozise/mnist-analysis:latest",
        file_outputs={
    ...
    with dsl.Condition(analysis.outputs["accuracy"] > baseline) as check_deploy:
        deploy = dsl.ContainerOp(
            name="deploy mar",
            image="molozise/mnist-deploy:latest",
